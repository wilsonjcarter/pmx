...
''')
    ...
    parser.add_argument('--norecursive',
                        ...
                        default=True,
                        action='store_false')
    parser.add_argument('--extra_mtp',
                        metavar='MTP',
                        dest='extra_mtp',
                        nargs='+',
                        default=None,
                        help='One or more extra MTP files to search for hybrid '
                        'residue parameters, in addition to the force-field '
                        'built-in database.  Use this to supply the .mtp files '
                        'generated by pmx mutate_nsaa for non-standard amino '
                        'acid (NSAA / PTM) mutations.  Files listed here take '
                        'precedence over the built-in database.')
    parser.add_argument('--supplement_bonded',
                        metavar='ITP',
                        dest='supplement_bonded',
                        nargs='+',
                        default=None,
                        help='One or more bonded-supplement ITP files generated '
                        'by pmx mutate_nsaa (prepare_nsaa_ff) for non-standard '
                        'amino acid (NSAA / PTM) mutations.  These files supply '
                        'cross-boundary AMBER<->GAFF bonded terms and ITP-internal '
                        'GAFF terms needed when filling B states for NSAA hybrids.')
...
def main(args):
    ...
    pmxtop, pmxitps = gen_hybrid_top(topol=topol, recursive=recursive,
                                     ...
                                     extra_mtp_files=args.extra_mtp,
                                     supplement_bonded_files=args.supplement_bonded)
